...
- **v3.0 M2 + M3.6 completion (engine + SSOT closure)**: Closed Milestones M2 and M3 in a single bundled cycle. **M2.1 multi-agent stubs**: `cli-parser.mjs` gained `--all-agents` and `--idioms` alias (kept `--idiom` for back-compat); the wizard's single-select IDE step became a `safeCheckbox` multi-select covering Claude, Cursor, Copilot, Gemini, Codex, Windsurf, and Roo Code; `writeAgentConfig` fixed the Gemini target (`AI_INSTRUCTIONS.md` → `GEMINI.md`), added the Codex target (root `AGENTS.md` stub), and added `alwaysApply: true` to the Cursor frontmatter; a new `buildAgentStub()` helper renders thin 5-line pointer files for Codex and Gemini; `getActiveAgents` dedupes and excludes the `none` sentinel. **M2.2**: validated catalog-only idiom injection (zero BYO residuals). **M2.3 assembler rewrite**: `instruction-assembler.mjs` refactored to a single `SKILL_CATALOG` constant (13 entries) as the source of truth for the dynamic Skill Registry, with `buildSkillRegistry` now filtering by category and rendering grouped output; fixed stale `.ai/instructions/core/staff-dna.md` pointers in the manifesto and DNA-GATE to `.ai/skills/staff-dna.md`; repointed the Agent Roles reference to `.ai/instructions/templates/agent-roles.md`. **Critical gap fix**: `ruleset-injector.mjs` now copies `src/assets/skills/` → `.ai/skills/` (was missing entirely — AGENTS.md referenced skill files that were never populated in generated projects); `manifest-utils.computeHashes` swapped its `core/` scan for a `skills/` scan. **M2.4**: all 6 cycle command files (`sdg-*.md`) repointed their agent-roles reference to the new `templates/` location. **M3.6 SSOT closure**: `agent-roles.md` moved to `templates/`; `src/assets/instructions/core/` deleted entirely (16 files, ~90KB); `governance.mjs` `STANDARDS_PATH` repointed to `skills/code-style.md`; `sync-rulesets.mjs` maintainer hint updated; `audit-bundle.mjs` sovereign check repointed to `.ai/skills/staff-dna.md`. **Round-trip validation**: wiped `.ai/{skill,skills,instructions,commands,workflows}/`, ran `node src/engine/bin/index.mjs init .` non-interactively, regenerated `.ai/skills/` (11 skills + AGENTS.md) + `.ai/instructions/{templates,competencies,idioms,flavor}/` + `.ai/commands/`; extracted all 24 unique `.ai/*` paths from the generated AGENTS.md and `fs.existsSync`-verified every single one. 107/107 tests green. M1 + M2 + M3 all closed; unblocks M4 release prep (bump to 3.0.0, MIGRATION-v3.md, README v3).
